...
class TestLogicModel:
    ...
    def basic_model(self):
        ...
        mod = m.LogicModel(domainEl, domainUnPreds, domainBiPreds, use_sparse=True)
        mod.buildAll()
        return mod
    ...
    def test_unary_predicate_build(self, basic_model):
        ...
        if sp.issparse(mathematician_matrix):
            mathematician_matrix = mathematician_matrix.toarray()
    ...
    def test_binary_predicate_build(self, basic_model):
        ...
        # For sparse, this is a tuple of sparse matrices
        hates_tensor_tuple = basic_model.binaryPredicateTensors["hates"]
        hates_true = hates_tensor_tuple[0]
        hates_false = hates_tensor_tuple[1]
        ...
        assert hates_true[chris_idx, tom_idx] == 1.
        # Check implicit false: if true is 1, false is 0
        if hates_false is None:
             assert hates_true[chris_idx, tom_idx] == 1. # Implies false is 0
        else:
            assert hates_false[chris_idx, tom_idx] == 0.

        # Check hates(tom, john) -> True
        assert hates_true[john_idx, tom_idx] == 1.

        # Check hates(chris, chris) -> True
        assert hates_true[chris_idx, chris_idx] == 1.

        # Check hates(john, tom) -> False (not in list)
        assert hates_true[tom_idx, john_idx] == 0.
        if hates_false is not None:
            assert hates_false[tom_idx, john_idx] == 1.
    ...
    def test_add_to_domain_simple(self, basic_model):
        ...
        hates_tensor_tuple = basic_model.binaryPredicateTensors["hates"]
        # shape of sparse matrix is (N, N)
        assert hates_tensor_tuple[0].shape == (4, 4)
        if hates_tensor_tuple[1] is not None:
            assert hates_tensor_tuple[1].shape == (4, 4)
            # Default should be false
            assert hates_tensor_tuple[1][3, 0] == 1. # False that mary hates john
        else:
             # Implicitly false (0 in true matrix)
             assert hates_tensor_tuple[0][3, 0] == 0.
    ...
    def test_add_unary_predicate(self, basic_model):
        ...
        if sp.issparse(tall_matrix):
            tall_matrix = tall_matrix.toarray()
    ...
    def test_add_unary_predicate_probabilistic(self, basic_model):
        ...
        if sp.issparse(happy_matrix):
            happy_matrix = happy_matrix.toarray()
    ...
    def test_add_binary_predicate(self, basic_model):
        ...
        if likes_tensor[1] is None:
             assert likes_tensor[0][2, 0] == 0.
        else:
    ...
    def test_get_methods(self, basic_model):
        ...
        hates_pred_tuple = basic_model.getBinaryPredicate("hates")
        # tuple of sparse matrices
        assert len(hates_pred_tuple) == 2
        assert hates_pred_tuple[0].shape == (3, 3)
